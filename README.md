<div align="center">

# 🪙 Proyecto Final de Criptografía 

### Facultad de Ingeniería UNAM | Semestre 2026-1
<!-- 
![Criptografía](https://img.shields.io/badge/Materia-Criptograf%C3%ADa-blue?style=for-the-badge)
![Semestre](https://img.shields.io/badge/Semestre-2026--1-green?style=for-the-badge)
![UNAM](https://img.shields.io/badge/UNAM-Facultad%20de%20Ingenier%C3%ADa-gold?style=for-the-badge) 
-->

</div>

---

## 📋 Descripción

Este es el **proyecto final** para la materia de **Criptografía** del semestre 2026-1.

---

## 👥 Equipo de Desarrollo

<table align="center">
  <tr>
    <td align="center">
      <b>👨‍💻 Garcia Cerda Gerardo Daniel</b>
    </td>
  </tr>
  <tr>
    <td align="center">
      <b>👨‍💻 Hernandez Ruiz Leny Javier</b>
    </td>
  </tr>
  <tr>
    <td align="center">
      <b>👨‍💻 Silverio Martinez Andres</b>
    </td>
  </tr>
  <tr>
    <td align="center">
      <b>👨‍💻 Rios Valdes Oscar</b>
    </td>
  </tr>
</table>

<!-- 
---

## 🎯 Objetivos del Proyecto

- 🔒 Implementar algoritmos de cifrado y descifrado
- 🛡️ Aplicar conceptos de seguridad de la información
- 🔑 Desarrollar soluciones criptográficas prácticas
- 📊 Analizar y evaluar la seguridad de diferentes métodos

---

## 🚀 Instalación

```bash
# Clona este repositorio
git clone [URL_DEL_REPOSITORIO]

# Navega al directorio del proyecto
cd ProyectoFinal
```


---

## 💡 Uso

> [!NOTE]
> Instrucciones de uso serán actualizadas conforme avance el desarrollo del proyecto.

---

## 📚 Tecnologías y Herramientas

- 🔧 [Agrega las tecnologías que utilizarás]
- 📦 [Librerías y frameworks]
- 🛠️ [Herramientas de desarrollo]

---

## 📝 Documentación

La documentación completa del proyecto estará disponible en la carpeta `/docs`.

---

## 🤝 Contribuciones

Este proyecto es desarrollado exclusivamente por el equipo mencionado como parte del curso de Criptografía.

---

## 📄 Licencia

Este proyecto es desarrollado con fines académicos para la **Facultad de Ingeniería de la UNAM**.

---
--->
<div align="center">

**🏛️ Universidad Nacional Autónoma de México**  
**Facultad de Ingeniería**  
**Criptografía | 2026-1**

---

</div>